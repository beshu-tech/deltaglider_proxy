...
`docs/product/` follows Diátaxis strictly — every page is exactly ONE of: `tutorials/` (lessons, executed end-to-end before merge), `how-to/` (goal-named recipes), `reference/` (austere facts), `explanation/` (concepts). Manifest groups mirror the quadrants (Start here · 3× Guides · Reference · Concepts · Releases). The marketing site loads content by Vite glob over `docs/product/**/*.md` (`marketing/src/lib/docContent.ts`); the in-product viewer gets the same files from the binary (rust-embed of `docs/product/` in `src/demo.rs`, served session-gated at `GET /_/api/docs`, shaped by `demo/s3-browser/ui/src/docsBundle.ts`) — there is NO hand-maintained import list on either surface, and the markdown never enters the public JS bundle (`scripts/check-bundle-fingerprints.sh` fails the build if it does). Adding a doc = drop the .md + add its manifest entry; manifest.json must stay in parity with disk (CI: check-docs-registry.sh); `# validate` YAML blocks are linted. Running examples use ONE fixed cast (backends hetzner-fsn1/local-disk/aws-dr; buckets releases/db-archive/downloads; users ci-uploader/backup-bot/dana; group Engineering) — never invent new example names. Marketing 301s for old slugs live in marketing/astro.config.mjs REDIRECTS (sitemap filter derives from it). New docs: pick the quadrant first; mixed-type pages are a regression.
...
- `SessionStore` (in `session.rs`) — in-memory session store with OsRng token generation, configurable TTL (`DGP_SESSION_TTL_HOURS`, default 4h), IP binding, max 10 sessions per principal (its own oldest evicted) and 1024 in total (browser-lift sessions evicted before admin ones).
...
It does NOT encrypt the config DB (S8). The SQLCipher key is `DGP_CONFIG_DB_KEY` (required + identical on every node when `config_sync_bucket` is set — boot refuses otherwise) else the key file `<db>.key` (0600, generated on first boot, never regenerated when present-but-empty) — `src/config_db/key.rs`. `ConfigDb::open_with_keys` tries the primary key, then FALLBACKS (`DGP_CONFIG_DB_KEY_PREVIOUS` = rotation; a key file that the env key replaces; the bootstrap hash = the pre-S8 key) and re-encrypts a fallback-opened DB to the primary via `rekey_file` (rekey a COPY → verify → atomic rename; the original is never at risk). A boot that re-keyed the DB with a sync bucket set parks an upload (`config_db_sync::park_upload`) so the synced copy moves to the new key while the old one is still a fallback. The sync download path uses the same keyring (a legacy-hash peer upload is accepted and re-keyed; another key → `Err` naming `DGP_CONFIG_DB_KEY`). No key opens the DB → `.db.bak` park + mismatch lock + recovery wizard (`recover-db` accepts a DB key or a legacy hash, read-only probe); a later boot with the right key promotes the bak (Err branch, or Ok branch when the live DB is empty).
...
**Embedded browser sessions (`dgp_session`):** S3 secrets stay off `localStorage`; reload restores via the cookie + `GET /_/api/admin/session/s3-credentials`. **AdminGui** sessions (bootstrap password, `login-as` for IAM admins, OAuth for EFFECTIVE admins — others get S3BrowserLift) can call the full admin API; every admin request re-checks that the session's principal is still an enabled admin in the live IAM index (`admin_gui_session_ok`). **S3BrowserLift** sessions (`POST /_/api/admin/session/browser-connect` for IAM non-admins, `POST …/open-browser-connect` when `authentication: none`) only reach logout, `GET /session`, and s3-credentials — config/IAM/etc. return **403** (`admin_session_required`). `GET /_/api/admin/session` returns `{ valid, admin_gui }` so the UI can enable folder usage scan only when `admin_gui` is true. **Proxy restart** clears all sessions (in-memory store). Same-origin only: do not enable permissive CORS with cookie auth.

The admin IA is **5 groups / 17 leaves** (`components/adminNavigation.tsx` `ADMIN_IA`; see `docs/plan/admin-ui-taxonomy.md`): Observability(dashboard/trace/delta-efficiency/audit/system-logs) · Access(credentials/users/groups/external-auth/admission) · Storage(backends/buckets/**Jobs**) · Integrations(event-delivery/event-outbox) · System. **Jobs** is ONE screen for replication+lifecycle rules and one-off reencrypt/migrate jobs (unified table over GET /jobs, drawer with Definition/Runs/Failures, TWO storage-section editors behind one dirty bar with a SEQUENTIAL apply queue). **System** is ONE page stacking Listener&TLS/Caches/Limits/Logging/Sync/Backup cards, each with its own dirtyKey + inline dirty bar. Section panels share TWO content-column widths (`CONTENT_FORM`/`CONTENT_WIDE` in `shared-styles.ts`), not per-panel maxWidth. Every old URL scheme remaps via `src/adminPathRemap.ts` (exhaustive table, regression-tested). `SidebarEntry` carries `dirtyKeys[]` (multi-editor leaves) and `applyKey` (⌘S dispatch). There is NO page-level save-model badge (a page mixes staged editors and immediate buttons): staged editors show `StickyDirtyBar` with a "Review & apply" button; immediate buttons name their action. Fields that a `DGP_*` env var controls render read-only with a "from env" badge — ONE rule in `FormField`, fed by `GET /config` → `env_overrides` (`src/config/env_overrides.rs`, secrets carry no value). Pure job-view logic in `src/jobsView.ts`. A first-run setup wizard at `/_/admin/setup` covers the zero-to-working flow.
...
Property tests via `proptest` (dev-dep) live in the same module as the pure functions they exercise (see `src/security.rs` for `validate_bucket_name` / `bucket_name_is_ip_like`). Coverage is collected via `cargo-llvm-cov` in the nightly (`scripts/coverage.sh`: lib tests, then every integration test, in one instrumented build; `show-env` makes the spawned proxy children write profiles, and the harness stops children with SIGTERM under coverage because a SIGKILLed child writes none). It reports lib-only vs lib + integration — use it as signal, not a gate. Race tests (`tests/race_conditions_test.rs`) fire N requests with `join_all` on check-then-act paths; the spool budget and the reference-lock protocol have model proptests (`budget_model_tests`, `lock_protocol_model_tests`). The differential contract test `src/api/s3s_contract_tests.rs` feeds raw requests through our admission → SigV4 → authz middleware AND the real s3s service (hooks in `src/api/s3s_hooks.rs`) and fails when the two disagree on identity, bucket, key or action — extend its table when you touch request parsing. Fuzz targets live in `fuzz/` (own workspace; entry points in `src/fuzz_entry.rs`, seeds run as a lib test): `scripts/fuzz.sh <target> [secs]`; the nightly `fuzz` job runs each for 5 min.

**Prod-config regression** is two-layered: `prod_shape_tests` in `src/config.rs` (lib tests, in the CI gate) validate `tests/fixtures/prod_shape_config.yaml` — a SANITIZED, structure-true snapshot of the production config (declarative IAM + conditions + `${iam:username}`, OIDC + mapping rules, s3 + encrypted-filesystem backends, routing, public_prefixes, replication/lifecycle rules) — through parse, reconciler validation, auth classification, and canonical-export round-trip; `tests/prod_shape_matrix_test.rs` (PR gate, filesystem; MinIO variant too) BOOTS a proxy from the same fixture (`TestServer::from_config_document`, only machine-specific values changed) and runs a request matrix over every route, IAM user, public prefix and job kind. Test fixtures come from the write path: the source guard `test_fixtures_come_from_the_write_path` (src/lib.rs) refuses `FileMetadata { .. }` literals in test code and hand-written DG metadata on stored objects in integration tests. Update the fixture deliberately when prod adopts new features; keep it sanitized (public repo). `scripts/test-prod-config.sh` (local-only, pre-release) boots the current branch's release binary against the REAL prod state (backup zip, or a clone of `/private/tmp/dgp-prod-local` incl. the `.deltaglider_bootstrap_hash` sidecar) with expectations derived dynamically from the prod YAML itself; it only ever writes to filesystem-routed buckets, never to remote backends.
...
- **Test the observable contract, not the implementation.** Integration tests cost a TestServer spawn (~200ms + MinIO when S3). Unit-test anything that doesn't need the whole HTTP/SigV4/storage stack; reserve integration tests for request-pipeline seams, concurrency races, and cross-subsystem invariants (metadata cache vs storage, admission vs auth, etc.). The hygiene pass in `5f38941` deleted 659 LOC of integration tests that were re-covering pure-function logic. **But demote only when the assertion is TRULY pure**: a test that spawns a server but asserts real emergent behaviour — xdelta3 actually compressing (`savings_test`), on-disk corrupt xattrs degrading gracefully (`metadata_validation_test`), s3s XML-body parsing → 400 (`error_test`) — is NOT demotable; a filename/header saying "runs the same operations as X" is a hint to VERIFY, not a demotion warrant (both `s3_backend_test` and `public_prefix_list_test` were already trimmed to their irreducible seams). The pure-mapping half CAN move: `engine_error_to_s3s`'s NoSuchKey→code mapping is a unit test; the wire round-trip that proved it was redundant.
...
- **Use `cargo-llvm-cov` output to steer.** Coverage numbers are signal, not a gate. If a new hot-path file lands at 0 line coverage that's a flag; don't let it merge without explaining why (e.g. MinIO-only branches intentionally excluded from the unit run). Coverage + supply-chain (cargo-deny/audit) now run in `test-all-nightly.yml`, NOT on the PR gate — the PR gate is correctness-only. The PR CI uses **build-once-consume-many**: `build-ui` and `build-bin` produce the UI `dist/` and release binary as artifacts once; every downstream job `needs:` + downloads them instead of re-running `npm ci` / `cargo build --release`. Don't reintroduce a per-job UI/binary build.
