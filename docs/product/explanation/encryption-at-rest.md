...
A proxy-AES backend cannot simply swap its `key`. If the old key were gone, every object written under it would become unreadable — AES-GCM doesn't degrade gracefully, it authenticates or it fails. That is why a key change keeps the previous key in the legacy slot described below. Supporting transparent multi-key rotation would mean a key ring, per-object key resolution against an unbounded set, and a quiet pile of complexity in the hottest read path. The proxy refuses that trade.
