...
When `DGP_TRUSTED_PROXY_CIDRS` is set, the proxy reads only `X-Forwarded-For`, and only from a connection that comes from one of those networks. It walks the header from right to left and takes the first address that is not in a trusted network. When every address in the header is trusted, it takes the leftmost one. When the header is absent, it takes the address of the connection. It does not read `X-Real-IP` in this mode, because that header has no chain of hops: a reverse proxy that passes the client's copy through unchanged looks the same as one that wrote the header itself, so a client could choose its own address. Configure the reverse proxy to send `X-Forwarded-For`.
