...
- With `format: raw` and with `format: slack`, the dispatcher stores the result of each target (each webhook URL, and in bot-token mode each Slack channel) in the `event_deliveries` table (outbox row id, endpoint id, status, attempts, last error). The endpoint id is a hash of the URL or of the channel id, so the table never holds a URL token. A failing endpoint does not stop the posts to the other endpoints. A retry, and a requeue, post only to the endpoints that have not yet returned 2xx. The row's `last_error` joins the errors of the endpoints that failed in the last attempt. When you add an endpoint, the retries of an unfinished row also post to it.
