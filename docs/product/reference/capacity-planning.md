...
- **Listing-size cache (`DGP_LIST_SIZE_CACHE_MB`, default 32 MB):** remembers the original size and ETag of each stored delta that this proxy wrote or read, so that a listing can report them without a metadata request per object. An entry takes about 275 bytes for a 60-byte object key (the entry holds the endpoint, the bucket, the key, both ETags and the size), so the default holds about 120,000 objects.
