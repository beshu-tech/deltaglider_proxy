...
const { findEnvOverride, envOverrideText, envOverrideSource, envOverrideHelp, backendEncryptionEnvPath } = await import(
...
// ── Block members and the badge/help wording ──
const unsetMember = {
  env: 'DGP_S3_REGION',
...
// ── Every env-controlled YAML path the server reports has a UI consumer ──
...
const ui = (await sources(new URL('../src/', import.meta.url))).join('\n');
...
console.log('env-overrides consumer checks passed');
