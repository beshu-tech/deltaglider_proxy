// Guards the demo data generator's hand-rolled ZIP byte-writer (src/makeZip.ts)
// against silently producing CORRUPT zips — a header offset / u32-packing /
// CRC mistake would break "download opens".
import assert from 'node:assert/strict';
import { test } from 'vitest';
import { crc32, makeZip, makeReleaseZip } from '../makeZip';

function parseZip(bytes: Uint8Array): { data: Uint8Array; name: string } {
...
test('CRC-32: known IEEE test vectors', () => {
...
test('a stored-entry zip is structurally valid', () => {
...
test("empty content and multi-byte UTF-8 entry name don't corrupt the structure", () => {
  parseZip(makeZip('a/b.txt', ''));
  parseZip(makeZip('ünïcödé/nâme.txt', 'çontent ✓'));
});

test('versioned zips: valid, and near-identical (the delta-demo property)', () => {
