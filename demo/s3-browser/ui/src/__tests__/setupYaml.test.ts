...
import { generateSetupYaml, setupEnvControl, setupStepComplete, type SetupYamlInput, type SetupEnvControl, type SetupStepInput } from '../setupYaml';
import type { EnvOverride } from '../envOverrides';

const NO_ENV_CONTROL: SetupEnvControl = { adminAccessKeyId: false, adminSecretKey: false, backendKind: null };
...
function serverParse(text: string): unknown {
  assert.ok(!/(^|[^$])(\$\$)*\$(?!\$)/.test(text), `undoubled $ in:\n${text}`);
  return parse(text.replace(/\$\$/g, '$'));
}

const base: SetupYamlInput = {
...
test('every string the operator types survives emit → parse byte-for-byte', () => {
  ...
  for (const v of nasty) {
    // access section
    const a = serverParse(generateSetupYaml({ ...base, adminAccessKeyId: v, adminSecretKey: v })) as {
      access: { access_key_id: string; secret_access_key: string };
    };
    ...
    // filesystem path
    const f = serverParse(generateSetupYaml({ ...base, fsPath: v })) as { storage: { filesystem: string } };
    ...
    ) as { storage: { s3: string; region: string; access_key_id: string; secret_access_key: string } };
...
test('unchanged shape for ordinary input', () => {
...
test('defaults omitted: us-east-1 region, path-style on, no creds → no access section', () => {
...
test('env-controlled answers (#92): counted as given, left out of the YAML', () => {
  const envList: EnvOverride[] = [
...
test('setupStepComplete: step gating with and without env control', () => {
  const isAbs = (p: string) => p.startsWith('/') || /^[A-Za-z]:[\\/]/.test(p);
  const empty: SetupStepInput = { backendKind: 's3', fsPath: '', adminAccessKeyId: '', adminSecretKey: '', adminSecretKeyConfirm: '' };
  const envList: EnvOverride[] = [
    { env: 'DGP_ACCESS_KEY_ID', yaml_path: 'access.access_key_id', secret: false, value: 'AKIAENV', set: true },
    { env: 'DGP_SECRET_ACCESS_KEY', yaml_path: 'access.secret_access_key', secret: true, set: true },
    { env: 'DGP_S3_ENDPOINT', yaml_path: 'storage.backend.type', secret: false, value: 's3', set: true },
  ...
  const idOnly: SetupEnvControl = { ...NO_ENV_CONTROL, adminAccessKeyId: true };
