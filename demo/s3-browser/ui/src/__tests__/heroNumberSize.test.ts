...
test('heroFontPx: bounds, monotonicity, and width fit across the legitimate range', () => {
...
test('heroFontPx: ceiling — short values still render huge', () => {
...
test('heroFontPx: floor — absurdly long string in a narrow column clamps to 40, not below', () => {
  assert.equal(heroFontPx(13, 240), 40, '13 chars at 240px clamps to the 40px floor');
});

test('heroFontPx: percent-lead branch shape ("95.1" — dot budgeted like a separator)', () => {
  assert.ok(heroFontPx('95.1'.length, 240) >= 40 && heroFontPx('95.1'.length, 240) <= 138);
});

test('heroFontPx: unmeasured / bogus widths fall back instead of exploding', () => {
