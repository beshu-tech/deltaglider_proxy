...
import {
  ...
  type EventDeliveryWire,
  type SlackRouteRow,
} from '../components/webhookDeliveryPayload';
...
function buildFromWire(wire: EventDeliveryWire, mutate?: (f: WebhookFormState) => void) {
  const form = formFromWire(wire, mkIdGen());
  if (mutate) mutate(form);
  return buildPayloadFromForm(form);
}
function headers(res: ReturnType<typeof buildFromWire>): Record<string, string> {
  if (!res.ok || !res.body) throw new Error('expected an ok result with a body');
  return res.body.event_delivery!.webhook_headers as Record<string, string>;
}
function ed(res: ReturnType<typeof buildFromWire>) {
  if (!res.ok || !res.body) throw new Error('expected an ok result with a body');
  return res.body.event_delivery!;
}

test('legacy webhook_url folds into the endpoint list (deduped, legacy first)', () => {
  {
...
test('legacy webhook_url is always cleared on save; list preserved', () => {
  const res = buildFromWire({ webhook_url: 'https://legacy.example/h' });
  assert.ok(res.ok);
  assert.equal(ed(res).webhook_url, null, 'legacy webhook_url cleared');
  assert.deepEqual(ed(res).webhook_urls, ['https://legacy.example/h']);
});

test('header DELETE emits explicit null (RFC 7396) for a REMOVED row', () => {
  const res = buildFromWire({ webhook_headers: { Authorization: SENTINEL, 'X-Env': SENTINEL } }, (f) => {
  ...
  const rt = JSON.parse(JSON.stringify(res.body)) as { event_delivery: { webhook_headers: Record<string, unknown> } };
  assert.ok('X-Env' in rt.event_delivery.webhook_headers, 'null delete-key survives JSON.stringify');
});

test('untouched masked secret passes through as sentinel (server restores)', () => {
  const res = buildFromWire({ webhook_headers: { Authorization: SENTINEL } });
  assert.ok(res.ok);
  assert.equal(headers(res)['Authorization'], SENTINEL);
});

test('retyped secret + new header pass through with real values', () => {
...
test('RENAME a still-masked header is BLOCKED (adversarial #2)', () => {
  ...
  assert.ok(
    !res.ok && res.errors.some((e) => /re-enter/i.test(e) || /re-type/i.test(e)),
    'error must tell operator to re-type the value',
  );
});

test('rename WITH a re-typed value is allowed; old name deleted', () => {
  const res = buildFromWire({ webhook_headers: { Authorizaton: SENTINEL } }, (f) => {
...
test('in-progress empty rows are ignored, not errors', () => {
  const res = buildFromWire({}, (f) => {
    f.urlRows = [{ id: 'a', url: '' }];
    f.headerRows = [{ id: 'b', name: '', value: '', origName: '', masked: false }];
  });
  assert.ok(res.ok, `empty in-progress rows must not error, got ${JSON.stringify(!res.ok && res.errors)}`);
  assert.deepEqual(ed(res).webhook_urls, []);
});

test('enabling with zero endpoints is rejected (usability trap)', () => {
  const res = buildFromWire({}, (f) => {
    f.enabled = true;
    f.urlRows = [];
  });
...
test('duration validation: simple + compound humantime (1h30m) + word forms', () => {
...
test('invalid endpoint URL + invalid header name rejected', () => {
...
test('format: raw still round-trips unchanged (no slack fields leak as set)', () => {
...
test('slack bot token: untouched (masked) → passes through as the sentinel', () => {
...
test('slack bot token: retyped → real value flows through', () => {
  const res = buildFromWire({ format: 'slack', slack_bot_token: SENTINEL, slack_channel: '#deploys' }, (f) => {
...
test('slack bot-token mode needs a channel', () => {
  const res = buildFromWire({
    format: 'slack',
  ...
  assert.ok(
    !res.ok && res.errors.some((e) => /needs a channel/i.test(e)),
    'error must mention the missing channel',
  );
});

test('a masked (untouched) token also counts as bot mode → still needs channel', () => {
...
test('slack webhook mode (no token), enabled, no URL → must fail', () => {
  ...
  assert.ok(
    !res.ok && res.errors.some((e) => /webhook url/i.test(e) || /no slack incoming webhook/i.test(e)),
    'error must mention the missing Slack webhook URL',
  );
});

test('slack webhook mode with a hooks.slack.com URL → ok', () => {
  const res = buildFromWire({ format: 'slack', webhook_urls: ['https://hooks.slack.com/services/T/B/x'] }, (f) => {
    f.enabled = true;
  });
  assert.ok(res.ok, JSON.stringify(!res.ok && res.errors));
  assert.equal(ed(res).format, 'slack');
...
test('slack notify kinds: at least one must be selected', () => {
  const res = buildFromWire({ format: 'slack', webhook_urls: ['https://hooks.slack.com/services/T/B/x'] }, (f) => {
    f.slackNotifyKinds = [];
  });
...
test('slack notify kinds + globs round-trip; empty glob row rejected', () => {
  ...
  const bad = buildFromWire({ format: 'slack', webhook_urls: ['https://hooks.slack.com/services/T/B/x'] }, (f) => {
...
test('slack cosmetic fields (webhook mode) flow through; empty → null', () => {
  const res = buildFromWire({ format: 'slack', webhook_urls: ['https://hooks.slack.com/services/T/B/x'] }, (f) => {
...
test('route round-trip: wire → form → payload (bot-token mode)', () => {
  const res = buildFromWire({
    format: 'slack',
    slack_bot_token: SENTINEL,
...
test('empty-channel route is dropped (in-progress / blank)', () => {
  const res = buildFromWire({ format: 'slack', slack_bot_token: SENTINEL, slack_channel: '#default' }, (f) => {
    f.slackRoutes = [
...
test('a half-filled route (bucket but no channel) is an error, not silently dropped', () => {
  const res = buildFromWire({ format: 'slack', slack_bot_token: SENTINEL, slack_channel: '#default' }, (f) => {
...
test('prefix_globs round-trip (trimmed, blanks dropped)', () => {
  const res = buildFromWire({ format: 'slack', slack_bot_token: SENTINEL, slack_channel: '#default' }, (f) => {
    f.slackRoutes = [
      {
        id: 'rt0',
  ...
  assert.deepEqual(
    ed(res).slack_routes,
    [{ channel: '#ci', prefix_globs: ['builds/**', 'dist/*'] }],
    'globs trimmed, blanks dropped',
  );
});

test('routes present but NOT bot-token mode → surfaced error', () => {
  const res = buildFromWire({ format: 'slack', webhook_urls: ['https://hooks.slack.com/services/T/B/x'] }, (f) => {
    f.enabled = true;
...
test('routes + empty fallback channel coexist (allowed)', () => {
  const res = buildFromWire({ format: 'slack', slack_bot_token: SENTINEL }, (f) => {
    f.slackChannel = ''; // no fallback
    f.slackRoutes = [{ id: 'rt0', name: '', bucket: '', prefixGlobs: [], channel: '#ci' }];
  });
  assert.ok(res.ok, `routes + empty fallback channel must be allowed, got ${JSON.stringify(!res.ok && res.errors)}`);
...
// ─────────────────────────────────────────────────────────────────────────
function route(name: string | null, bucket: string | null, globs: string[] | null, channel: string): SlackRouteRow {
  return {
    id: (name ?? '') + channel,
    name: name ?? '',
    bucket: bucket ?? '',
...
test('resolveSlackChannelsPreview: no routes → fall back to the single channel', () => {
...
test('resolveSlackChannelsPreview: bucket-scoped route matches its bucket, misses others', () => {
...
test('resolveSlackChannelsPreview: fan-out — an event matches multiple routes → all channels (deduped)', () => {
  const routes = [
    route('any', '', [], '#all'),
...
test('resolveSlackChannelsPreview: prefix-glob match (** crosses /, * within segment)', () => {
...
test('resolveSlackChannelsPreview: no match + no fallback → no channel', () => {
