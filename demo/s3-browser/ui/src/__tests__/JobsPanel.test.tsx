...
describe('run-now', () => {
  test('POSTs run-now, accepts the 202, refetches, and polls until the run settles', async () => {
    ...
    expect(within(jobRow()).queryByRole('button', { name: 'Run now' })).not.toBeInTheDocument();
    ...
    expect(within(jobRow()).getByRole('button', { name: 'Run now' })).toBeInTheDocument();
  });
  ...
  test('a disabled rule offers "Run once" (runs a single time, does not enable it)', async () => {
    ...
    await mount();
    expect(
      within(jobRow()).getByRole('button', { name: 'Run this rule once now — does not enable or resume it' }),
  ...
  test('run-now waits while the replication editor has unsaved edits', async () => {
    ...
    await waitFor(() =>
      expect(within(jobRow()).getByRole('button', { name: 'Run now' })).toBeDisabled(),
    );
    expect(within(jobRow()).getByRole('button', { name: 'Run now' })).toHaveAttribute(
      'title',
