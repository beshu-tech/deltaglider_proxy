...
import { sourceIpMatch, sourceIpText, parseSourceIpLines, sourceIpProblem } from '../sourceIpField';

test('parseSourceIpLines: lines are trimmed; blank lines are dropped', () => {
...
test('sourceIpMatch chooses source_ip vs source_ip_list', () => {
...
test('sourceIpText: loading a rule shows its entries, whichever key it used', () => {
...
test('sourceIpMatch preserves the original key when the value is unchanged', () => {
...
test('sourceIpProblem: client-side validation, with the line number (issue #92 review)', () => {
  for (const ok of [
    '',
    '203.0.113.5',
    '0.0.0.0/0',
