...
fn extract_request_info(request: &Request<Body>) -> OwnedRequestInfo {
    ...
    // Behind a listed proxy the client is the XFF hop the proxy saw (S13).
    // Without a CIDR list the first XFF element is client-written, so a
    // forged header would escape a `source_ip` deny: use the peer then.
    let peer_ip = request
        .extensions()
